    ...
    <PaginatedResults v-if="results" :totalPages :initialPage="page" :totalItems :pageSize @changePage="onPageChange($event.page)">
      <DataTable v-if="results.length > 0" :value="results" size="small">
        <Column :header="t('general.name')">
          <template #body="{ data }">
            <RouterLink :to="`/forum/sub-category/${data.id}`">{{ data.name }}</RouterLink>
          </template>
        </Column>
...
import { getForumSubCategoryThreadsSubscriptions, removeForumSubCategoryThreadsSubscription, type ForumSubCategoryLite } from '@/services/api-schema'

const { t } = useI18n()
const route = useRoute()
const router = useRouter()
const results = ref<ForumSubCategoryLite[]>()
...
const load = () => {
  getForumSubCategoryThreadsSubscriptions({
    page: page.value,
...
const unsubscribe = (subCategoryId: number) => {
  removeForumSubCategoryThreadsSubscription(subCategoryId).then(() => {
