    ...
    <PaginatedResults v-if="results" :totalPages :initialPage="page" :totalItems :pageSize @changePage="onPageChange($event.page)">
      <DataTable v-if="results.length > 0" :value="results" size="small">
        <Column :header="t('general.name')">
          <template #body="{ data }">
            <RouterLink :to="`/artist/${data.id}`">{{ data.name }}</RouterLink>
          </template>
        </Column>
...
import { getArtistTitleGroupsSubscriptions, removeArtistTitleGroupsSubscription, type ArtistLite } from '@/services/api-schema'
...
const load = () => {
  getArtistTitleGroupsSubscriptions({
    page: page.value,
...
const unsubscribe = (artistId: number) => {
  removeArtistTitleGroupsSubscription(artistId).then(() => {
    showToast('', t('artist.unsubscription_successful'), 'success', 3000)
    load()
  })
}
