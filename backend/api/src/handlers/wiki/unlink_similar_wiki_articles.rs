...
#[utoipa::path(
    delete,
    operation_id = "Unlink similar wiki articles",
    tag = "Wiki",
    ...
    responses(
        (status = 200, description = "Successfully unlinked the wiki articles"),
    )
)]
pub async fn exec<R: RedisPoolInterface + 'static>(
    ...
    arc.pool.unlink_similar_wiki_articles(&link).await?;
