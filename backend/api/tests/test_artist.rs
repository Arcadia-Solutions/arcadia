...
async fn test_staff_can_edit_artist(pool: PgPool) {
    ...
    let req = test::TestRequest::put()
        ...
        .set_json(&serde_json::json!({
            "id": 1,
            "name": "Beatles, The",
            "description": "They are actually called 'The Beatles', but we decided to be weird with articles.",
            "pictures": [
    ...
    assert_eq!(response.name, "Beatles, The");
    assert_eq!(
        response.description,
        "They are actually called 'The Beatles', but we decided to be weird with articles."
    );
    assert_eq!(response.pictures.len(), 1);
    assert_eq!(response.pictures[0], "https://upload.wikimedia.org/wikipedia/commons/d/d8/The_Beatles_members_at_New_York_City_in_1964.jpg");
}
